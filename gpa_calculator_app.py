...
def assign_grade(percentage):
    """Step 2: Assign grade and grade point based on percentage"""
...
def calculate_weighted_point(grade_point, credit):
    """Step 3: Calculate weighted grade point"""
    return grade_point * credit

def calculate_semester_gpa(marks_data, semester_name):
    """
    Calculate semester GPA using credit-weighted averaging
    Returns: (GPA, total_weighted_points, total_credits)
    """
    ...
    for subject in curriculum:
        if subject['credit'] > 0:  # Only count components with credits
            ...
            if marks is not None and marks >= 0:
                ...
                weighted_point = calculate_weighted_point(grade_point, subject['credit'])
